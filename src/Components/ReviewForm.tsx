...
export default function ReviewForm({ roasterId, roasterSlug, roasterName, coffeeOptions }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If not logged in, prompt for email + send OTP
    if (!userId) {
      setNeedLogin(true);
      return;
    }
  ...
  const sendMagicLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      setErr(null);
      alert('Check your email for a login link. Once you’re back, press "Publish review".');
    } catch (e: any) {
    ...
    return (
      <div className="rounded-xl border border-gray-200 p-6 bg-white">
        ...
        <p className="mt-2 text-gray-600">
          Your review helps other coffee lovers discover {roasterName}. Want to add another?
        </p>
        <a
          href="/"
      ...
      <div className="flex flex-col gap-3">
        ...
        {!userId && (
          <div className="rounded-md border border-gray-200 p-3">
            ...
            <form onSubmit={sendMagicLink} className="mt-2 flex gap-2">
              <input
                type="email"
                value={email}
