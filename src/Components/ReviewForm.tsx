...
type Props = {
  ...
  roasterSlug: string;
  roasterName: string;
  // kept for compatibility with callers; not used now
  coffeeOptions?: string[];
};

const BREW_METHOD_PLACEHOLDER = 'Espresso, pour over, etc.';
...
export default function ReviewForm({
  ...
  const [coffeeName, setCoffeeName] = useState<string>('');   // text input now
  const [brewMethod, setBrewMethod] = useState<string>('');   // text input now
  const [origin, setOrigin] = useState<string>('');           // NEW optional field
  ...
  // save draft
  useEffect(() => {
    const draft = { roasterSlug, rating, coffeeName, brewMethod, origin, notes };
    try {
      localStorage.setItem(draftKey, JSON.stringify(draft));
    } catch {}
  }, [draftKey, roasterSlug, rating, coffeeName, brewMethod, origin, notes]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* Coffee (text) */}
      <div>
        <label className="block text-sm font-medium mb-1">Coffee (optional)</label>
        <input
          type="text"
          value={coffeeName}
          onChange={(e) => setCoffeeName(e.target.value)}
          placeholder="e.g., House Espresso, Los Rodriguez, etc."
      ...
      <div>
        ...
        <input
          ...
          placeholder={BREW_METHOD_PLACEHOLDER}
          className="w-full rounded-md border border-gray-300 px-3 py-2"
        />
      </div>
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g., Ethiopia (Yirgacheffe), Colombia, Blend"
          className="w-full rounded-md border border-gray-300 px-3 py-2"
        />
      </div>
